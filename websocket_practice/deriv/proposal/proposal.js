import DerivAPIBasic from 'https://cdn.skypack.dev/@deriv/deriv-api/dist/DerivAPIBasic';

const app_id = 1089; // Replace with your app_id or leave as 1089 for testing.
const connection = new WebSocket(`wss://ws.binaryws.com/websockets/v3?app_id=${app_id}`);
const api = new DerivAPIBasic({ connection });

const proposal_request = {
  proposal: 1,
  subscribe: 1,
  amount: 10,
  basis: 'payout',
  contract_type: 'CALL',
  currency: 'USD',
  duration: 1,
  duration_unit: 'm',
  symbol: 'R_100',
  barrier: '+0.1',
};

const proposalResponse = async (res) => {
  const data = JSON.parse(res.data);
  if (data.error !== undefined) {
    console.log('Error: %s ', data.error.message);
    connection.removeEventListener('message', proposalResponse, false);
    await api.disconnect();
  } else if (data.msg_type === 'proposal') {
    const proposalContainer = document.querySelector('#proposalContainer');
    proposalContainer.textContent = `Proposal: ${JSON.stringify(data.proposal)}`;
    console.log('Details: %s', data.proposal.longcode);
    console.log('Ask Price: %s', data.proposal.display_value);
    console.log('Payout: %f', data.proposal.payout);
    console.log('Spot: %f', data.proposal.spot);
  }
};

// const proposalResponse = async (res) => {
//   const data = JSON.parse(res.data);
//   if (data.error !== undefined) {
//     console.log('Error: %s ', data.error.message);
//     connection.removeEventListener('message', proposalResponse, false);
//     await api.disconnect();
//   } else if (data.msg_type === 'proposal') {
//     const proposalContainer = document.createElement('div');
//     proposalContainer.classList.add('proposal-container');

//     const detailsElement = document.createElement('p');
//     detailsElement.textContent = `Details: ${data.proposal.longcode}`;
//     proposalContainer.appendChild(detailsElement);

//     const askPriceElement = document.createElement('p');
//     askPriceElement.textContent = `Ask Price: ${data.proposal.display_value}`;
//     proposalContainer.appendChild(askPriceElement);

//     const payoutElement = document.createElement('p');
//     payoutElement.textContent = `Payout: ${data.proposal.payout}`;
//     proposalContainer.appendChild(payoutElement);

//     const spotElement = document.createElement('p');
//     spotElement.textContent = `Spot: ${data.proposal.spot}`;
//     proposalContainer.appendChild(spotElement);

//     document.body.appendChild(proposalContainer);
//   }
// };

const getProposal = async () => {
  connection.addEventListener('message', proposalResponse);
  await api.proposal(proposal_request);
};

const unsubscribeProposal = () => {
  connection.removeEventListener('message', proposalResponse, false);
};

const proposal = document.querySelector('#proposal');
proposal.addEventListener('click', getProposal);

const proposal_unsubscribe = document.querySelector('#proposal-unsubscribe');
proposal_unsubscribe.addEventListener('click', unsubscribeProposal);
